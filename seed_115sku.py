"""
seed_115sku.py -- A.O.M Cafe Coffee Bean Master SKU Seed Data (Updated)

Update summary:
1. Added 15 Guatemala estate/region batches from
   202407_All_Green_Coffee_Bean_COTIZACION_myCost_A.O.M_Muffins quote sheet.
   Covers: Acatenango, Huehuetenango, Coban, Atitlan, Antigua,
   New Oriente, Fraijanes, San Marcos regions.
2. Original 100 SKU + 15 new = 115 total, re-sorted by import region
   (continent: Africa -> Central/South America -> Asia -> Taiwan,
   then grouped by country) and renumbered sequentially 1-115.
3. All 20 Guatemala SKUs (5 original + 15 new) are now grouped together
   at SKU 46-65.
"""
...
def seed():
    """Import 115 SKU master records to database (delete-then-insert upsert,
