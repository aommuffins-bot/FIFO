...
USERS_DB = {
    ...
    "user1": {"username": "user1", "password_hash": hashlib.sha256("password123".encode()).hexdigest(), "role": "user", "store_id": 1},
}
...
INVENTORY_DB = {
    1: {"sku_no": 1, "name": "商品 A", "batches": [], "total_qty_g": 0.0},
    2: {"sku_no": 2, "name": "商品 B", "batches": [], "total_qty_g": 0.0},
}
TRANSACTIONS_DB = []

security = HTTPBearer()

# ==================== Pydantic 請求模型（與前端 JSON 欄位名稱完全一致）====================
...
# 登入維持 Query 參數（前端用 URL query string 傳送，兩邊一致）
@app.post("/auth/login")
...
# 進貨：改用 Pydantic Body Model 接收 JSON（與前端 fetch body 完全對應）
...
async def receive_stock(req: ReceiveRequest, current_user: dict = Depends(get_current_user)):
    ...
    TRANSACTIONS_DB.append({
        "type": "receive", "sku_no": req.sku_no, "qty_g": req.qty_g,
        "cost_per_100g": req.cost_per_100g, "total_amount": req.qty_g * req.cost_per_100g / 100,
        "supplier": req.supplier, "timestamp": new_batch["received_date"], "store_id": current_user["store_id"]
    })

    return {
        "status": "success", "message": "進貨成功：" + str(req.qty_g) + "g",
        "batch": new_batch, "new_total_qty_g": INVENTORY_DB[req.sku_no]["total_qty_g"]
    }

# 出貨：改用 Pydantic Body Model 接收 JSON
...
async def issue_stock(req: IssueRequest, current_user: dict = Depends(get_current_user)):
    ...
    if item["total_qty_g"] < req.qty_g:
        raise HTTPException(status_code=400, detail="庫存不足：目前 " + str(item["total_qty_g"]) + "g，需要 " + str(req.qty_g) + "g")
    ...
    TRANSACTIONS_DB.append({
        "type": "issue", "sku_no": req.sku_no, "qty_g": req.qty_g,
        "sell_price_ntd_per_100g": req.sell_price_ntd_per_100g,
        "total_amount": req.qty_g * req.sell_price_ntd_per_100g / 100,
        "channel": req.channel, "timestamp": datetime.utcnow().isoformat(),
        "store_id": current_user["store_id"], "batches_used": batches_used
    })

    return {
        "status": "success", "message": "出貨成功：" + str(req.qty_g) + "g",
        "batches_used": batches_used, "remaining_total_qty_g": item["total_qty_g"]
    }
...
async def export_inventory_report(store_id: int = Query(...), current_user: dict = Depends(get_current_user)):
    ...
    for sku_no, item in INVENTORY_DB.items():
        csv_content += str(sku_no) + "," + item["name"] + "," + str(item["total_qty_g"]) + "," + str(len(item["batches"])) + "\n"
...
async def export_transactions_report(store_id: int = Query(...), current_user: dict = Depends(get_current_user)):
    ...
    for t in TRANSACTIONS_DB:
        ...
        csv_content += date+","+t["type"]+","+str(t["sku_no"])+","+str(t["qty_g"])+","+str(unit_price)+","+str(t["total_amount"])+","+str(counterparty)+"\n"
    return PlainTextResponse(content=csv_content, media_type="text/csv",
