...
整合：auth.py、db_engine.py、fifo_engine_v2.py、models.py
"""
...
from models import ReceiveRequest, IssueRequest, InventoryItem, BatchItem, TransactionItem, LoginResponse, SuccessResponse
...
async def get_current_user(token: str = Depends(security)):
    try:
        ...
        # 使用 auth.py 的 authenticate 驗證使用者是否存在
        user = authenticate(username, "")  # 密碼留空，只驗證使用者存在
        if user is None:
...
async def login(username: str = Query(...), password: str = Query(...)):
    ...
    # 使用 auth.py 的 encode_token 產生 JWT
        ...
        return [
            {
                ...
                "oldest_batch_age_days": 30  # 簡化，實際可計算
            }
            for row in result
        ]
...
async def receive_stock(req: ReceiveRequest, current_user: dict = Depends(get_current_user)):
    """進貨登錄（FIFO 自動入帳）"""
    try:
        # 呼叫 fifo_engine_v2 的 receive_stock
        batch_id = fifo_receive(
            sku_no=req.sku_no,
            qty_g=req.qty_g,
            unit_cost_ntd_per_g=req.cost_per_100g / 100,  # 轉換為 per gram
        ...
        return {
            ...
            "new_total_qty_g": req.qty_g  # 簡化，實際可查詢
        }
...
async def issue_stock(req: IssueRequest, current_user: dict = Depends(get_current_user)):
    """出貨登錄（FIFO 自動扣帳）"""
    try:
        # 呼叫 fifo_engine_v2 的 issue_stock
        txn = fifo_issue(
            sku_no=req.sku_no,
            qty_g=req.qty_g,
            sell_price_ntd_per_g=req.sell_price_ntd_per_100g / 100,  # 轉換為 per gram
            channel=req.channel,
            store_id=current_user["store_id"],
            created_by=current_user.get("user_id")
        )
